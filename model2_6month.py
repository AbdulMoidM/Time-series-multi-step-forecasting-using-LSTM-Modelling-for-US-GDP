...
class Recession:
    def __init__(self, name, start_date, end_date):
        ...
        self.training_output = self.build_training_output(6)
    ...
    def build_training_input(self, month):
        ...
        # adding snp data
        current_date = self.start_date - relativedelta(months=+month_offset)
        training_snp = []
        while (current_date < self.start_date + relativedelta(months=+month_offset)):
    ...
    def build_training_output(self, month):
        ...
        training_gdp = []
        while (current_date < self.start_date + relativedelta(months=+2*month)):
            training_gdp.append(gdp[gdp['Date'] >= current_date]['Value'].iloc[0])
            current_date = current_date + relativedelta(months=+1)
...
def build_model(train_x, train_y):
	...
	# define parameters
	verbose, epochs, batch_size = 0, 1100, 4
...
n_input = 12 #meaning 12 month data before start of recession
trainingIn=array(training_input)
trainingOut=array(training_output)
#say we have recession_test_data
...
scoreSum=0
scoresSum=[0,0,0,0,0,0]
predictionsSum=[0,0,0,0,0,0]
for i in range(10):
...
#py.plot(firstArray, secondArray)
...
#     model = build_model(trainingIn, trainingOut)
#     yhat = model.predict(corona, verbose=0)
# 	# we only want the vector forecast
#     yhat = yhat[0]
#     print(yhat)
